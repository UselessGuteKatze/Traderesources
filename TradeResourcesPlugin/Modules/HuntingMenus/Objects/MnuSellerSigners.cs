    ...
    public class MnuSellerSigners : MnuActionsExt<MnuSellerSignersArgs> {
        public MnuSellerSigners(string moduleName, string mnuName, string mnuTitle) : base(moduleName, mnuName, mnuTitle)
        {
            Enabled(rc => {
                ...
                var isUserRegistrator = rc.User.HasRole("TRADERESOURCES-Охотничьи угодья-Создание приказов", rc.QueryExecuter)/*rc.User.HasCustomRole("huntingobjects", "dataEdit", rc.QueryExecuter)*/;
            ...
            config
                .OnAction(Actions.View, action => action
                    ...
                    .OnRendering(re =>
                    {
                        ...
                        if (!isInternal)
                        {
                            if (tbSellerSigners.GetPair(re.User.GetUserXin(re.QueryExecuter), re.QueryExecuter, out var data))
                            {
                                var card = new Card();
                                card.AddComponent(new Heading(HeadingLevel.h3, $"{tbSellerSigners.flSellerBin.Text}: {data.flSellerBin}"));
                                card.AddComponent(new Heading(HeadingLevel.h3, $"{tbSellerSigners.flSignerBin.Text}: {data.flSignerBin}"));
                                card.Actions.Add(new Link
                                {
                                    Text = re.T("Изменить"),
                                    Controller = ModuleName,
                                    Action = MenuName,
                                    RouteValues = new MnuSellerSignersArgs { Id = data.flId, MenuAction = Actions.Edit }
                                });
                                re.Form.AddComponent(card);
                            } else
                            {
                                re.Form.AddComponent(new Link
                                {
                                    Text = re.T("Добавить"),
                                    Controller = ModuleName,
                                    Action = MenuName,
                ...
                .OnAction(Actions.Edit, action => action
                    ...
                    {
                        var tbSellerSigners = new TbSellerSigners();
                        tbSellerSigners.GetPair(re.User.GetUserXin(re.QueryExecuter), re.QueryExecuter, out var data);
