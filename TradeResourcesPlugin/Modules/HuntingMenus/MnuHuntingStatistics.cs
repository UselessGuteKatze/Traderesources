...
namespace TradeResourcesPlugin.Modules.HuntingMenus {
    public class MnuHuntingStatistics : FrmMenu {

        private static MemoryCache _memCache = new MemoryCache(new MemoryCacheOptions());

        public MnuHuntingStatistics(string moduleName) : base(nameof(MnuHuntingStatistics), "Статистика")
        {
        ...
        private Card tradeCountGraphs(SelectResultProxy<QueryJoin<TbTrades, TbObjects>> rows)
        {
            var dateGroupedValues = rows
                    .Where(r => new[] { RefTradesStatuses.Held, RefTradesStatuses.Cancel }.Contains(r.GetVal(t => t.L.flStatus)))
        ...
        private Card tradeSuccessPie(SelectResultProxy<QueryJoin<TbTrades, TbObjects>> rows)
        {
            ...
            var refSt = new RefTradesStatuses();
        ...
        {
            var statusGroupedValues = rows
                    .Where(r => r.GetVal(t => t.L.flStatus) == RefTradesStatuses.Wait).Count();
        ...
        {
            var statusGroupedValues = rows.Where(r => r.GetVal(t => t.L.flStatus) == RefTradesStatuses.Held && r.GetValOrNull(t => t.L.flCost).HasValue).Sum(r => r.GetVal(t => t.L.flCost));
