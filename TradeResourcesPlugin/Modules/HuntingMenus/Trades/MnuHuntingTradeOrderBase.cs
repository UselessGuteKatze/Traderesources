...
namespace TradeResourcesPlugin.Modules.HuntingMenus.Trades {
    public class MnuHuntingTradeOrderBase : MnuOrderBaseV2<HuntingTradeModel, TradeOrderTypes, HuntingTradeOrderQueryArgs> {

        public MnuHuntingTradeOrderBase(string moduleName, string menuName, OrderStandartPermissions perms)
        ...
        public override Task<ActionItem[]> GetActions(ActionEnv<HuntingTradeOrderQueryArgs> env)
        {
            ...
            switch (env.Args.MenuAction)
            {
                case Actions.ViewOrder:
                    ...
                    if (trade != null)
                    {
                        ret.Add(new ActionItem(env.T("Перейти к объекту"), new ActionRedirectData(ModuleName, MnuHuntingObjectView.MnuName, new HuntingObjectViewArgs { MenuAction = "view", Id = trade.flObjectId })));
                    }
                    break;
        ...
        public override IWizardFormBuilderWithStep<HuntingTradeOrderQueryArgs, ModelOrderArgs<TradeOrderTypes, HuntingTradeOrderQueryArgs>, HuntingTradeModel>
            EditModel(IWizardFormBuilderWithFinishButton<HuntingTradeOrderQueryArgs, ModelOrderArgs<TradeOrderTypes, HuntingTradeOrderQueryArgs>, HuntingTradeModel> wizard)
        {
            return wizard
                ...
                .OnRendering(re => {
                    ...
                    tbRev.flPublications.RenderCustom(re.Panel, re.Env, trade.flPublications);
                ...
                .OnValidating(ve => {
                    ...
                    var checkFields = new Field[] {
                        tbRev.flPublications
                    };
                    checkFields.Each(f => f.Validate(ve.Env));

                    //var flPublications = (Dictionary<string, object>[])tbRev.flPublications.GetValue(ve.Env);

                    //if (flPublications.Select(x => x[PublicationsEditor.Fls.flNewspaperId.ToString()].ToString().Substring(0, 2)).Distinct().Count() < 2)
                    //{
                    //    ve.Env.AddError(tbRev.flPublications.FieldName, ve.Env.T("Нужно указать хотя-бы по одной статье на государственном и русском языках."));
                    //}
                ...
                .OnProcessing(pe => {
                    ...
                    var flPublications = (Dictionary<string, object>[])tbRev.flPublications.GetValue(pe.Env);

                    flPublications.Each(x => {
                        var tbRefNewspapers = new TbRefNewspapers();
                        tbRefNewspapers.AddFilter(t => t.flId, x["flNewspaperId"].ToString());
                        var langId = tbRefNewspapers.SelectScalar(t => t.flParentId, pe.Env.QueryExecuter);
                        x["flLanguageId"] = langId;
                    });

                    pe.Model.flPublications = flPublications;
        ...
        public override HuntingTradeModel GetModel(ActionEnv<HuntingTradeOrderQueryArgs> env, GetModelFrom from, TradeOrderTypes orderType, ITransaction transaction)
        {
            ...
            {
                GetModelFrom.Empty => new HuntingTradeModel()
                {
        ...
        public override TradeOrderTypes GetOrderType(ActionEnv<HuntingTradeOrderQueryArgs> env, GetOrderModelFrom orderModelFrom)
        {
        ...
        public override string GetWizardCancelRedirectUrl(ActionEnv<HuntingTradeOrderQueryArgs> env, TradeOrderTypes orderType, GetModelFrom modelFrom)
        {
            var urlHelper = env.RequestContext.GetUrlHelper();
            //TODO
            return modelFrom switch
            {
                GetModelFrom.Empty => urlHelper.YodaAction(ModuleName, nameof(MnuHuntingTradeOrderBase)),
                GetModelFrom.Entity => urlHelper.YodaAction(ModuleName, nameof(MnuHuntingTradeOrderBase), new HuntingTradeOrderQueryArgs { Id = env.Args.Id.Value, MenuAction = "view" }),
                GetModelFrom.Revision => urlHelper.YodaAction(ModuleName, MenuName, new HuntingTradeOrderQueryArgs { RevisionId = env.Args.RevisionId, MenuAction = Actions.ViewOrder }),
                _ => throw new NotImplementedException()
            };
        }

        public override ValidationResultBase IsCreateFromActionValid(ActionEnv<HuntingTradeOrderQueryArgs> env)
        {
            if (env.Args.Id == null)
            {
            ...
            if (!CanCreateOrder(env.Args.Id.Value, GetOrderType(env, GetOrderModelFrom.Entity), out var error, env))
            {
                if (env.Args.Id.HasValue) {
                    return new RedirectResult(ModuleName, MnuHuntingTradeView.MnuName, new MnuHuntingTradeViewArgs() { tradeId = env.Args.Id.Value }, error);
                }
                else {
        ...
        public static bool CanCreateOrder(int id, TradeOrderTypes orderType, out string error, ActionEnv<HuntingTradeOrderQueryArgs> env)
        {
        ...
        public override void SaveModel(int revisionId, HuntingTradeModel model, TradeOrderTypes orderType, ActionEnv<HuntingTradeOrderQueryArgs> env, ITransaction transaction, GetModelFrom modelFrom)
        {
        ...
        private void setObjectStatus(TradeOrderTypes orderType, HuntingTradeModel model, ActionEnv<HuntingTradeOrderQueryArgs> env, ITransaction transaction)
        {
            switch (orderType)
            {
                case TradeOrderTypes.Edit:
            ...
            var objectBloc = HuntingObjectBlocks.ActiveFree.ToString();
            if (orderType == TradeOrderTypes.Create)
            {
                objectBloc = HuntingObjectBlocks.ActiveToBeTraded.ToString();
            }
            if (orderType == TradeOrderTypes.Cancel)
            {
                objectBloc = HuntingObjectBlocks.ActiveFree.ToString();
            }
        ...
        private void setModel<TTradesTable>(DataModifingQueryProxy<TTradesTable> updateOrInsert, HuntingTradeModel model) where TTradesTable : TbTradesBase
        {
            updateOrInsert
                ...
                .Set(t => t.flAdditionalCondition, model.flAdditionalCondition)
        ...
        public override bool TryAcceptModel(int revisionId, HuntingTradeModel model, TradeOrderTypes orderType, ActionEnv<HuntingTradeOrderQueryArgs> env, ITransaction transaction, out string error)
        {
            var result = true;
        ...
        public override bool TryValidateModel(int revisionId, HuntingTradeModel model, TradeOrderTypes orderType, ActionEnv<HuntingTradeOrderQueryArgs> env, out string[] errors)
        {
            var result = true;
            var errorsList = new List<string>() { };
        ...
        private void setTradeChange(TradeOrderTypes orderType, HuntingTradeModel model, ActionEnv<HuntingTradeOrderQueryArgs> env, ITransaction transaction)
        {
        ...
        public override void ViewModel(RenderActionEnv<HuntingTradeOrderQueryArgs> env, HuntingTradeModel trade, TradeOrderTypes orderType, OrderStatus orderStatus)
        {
            ViewModel(env.Form, env, trade);
        }

        public static void ViewModel(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade)
        {
            ...
            viewModelPublicationsData(widget, env, trade);
        }

        public static void viewModelMainData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
        ...
        public static void viewModelContestData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
            ...
            tbRevs.flAdditionalCondition.RenderCustom(groupbox, env, trade.flAdditionalCondition, readOnly: true);
            tbRevs.flCommissionMembers.RenderCustom(groupbox, env, trade.flCommissionMembers, readOnly: true);

        }
        public static void viewModelPublicationsData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
            var groupbox = new Accordion(env.T("Публикации в СМИ"), collapsed);
            widget.AddComponent(groupbox);

            tbRevs.flPublications.RenderCustom(groupbox, env, trade.flPublications, readOnly: true);

        }

        public static void viewModelRequisitesData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        public static void viewModelAgreementData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        public static void viewModelSaleData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        public static void viewModelProtocolData(WidgetBase widget, FormEnvironment env, HuntingTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        protected override bool HasAccessToOrder(int revisionId, ActionEnv<HuntingTradeOrderQueryArgs> env)
        {
    ...
    public class HuntingTradeOrderQueryArgs : OrderQueryArgs {
