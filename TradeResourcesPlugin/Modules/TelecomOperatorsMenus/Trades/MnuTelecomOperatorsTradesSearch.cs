...
namespace TradeResourcesPlugin.Modules.TelecomOperatorsMenus.Trades {
    public class MnuTelecomOperatorsTradesSearch : FrmMenu {

        public MnuTelecomOperatorsTradesSearch(string moduleName) : base(nameof(MnuTelecomOperatorsTradesSearch), "Конкурсы")
        {
            ...
            OnRendering(re => {
                ...
                var isUserSeller = re.User.HasRole("TRADERESOURCES-Ресурсы связи-Для операторов связи-Создание приказов", re.QueryExecuter);

                var tbTrades = new TbTrades();

                if (isUserSeller && !isInternal)
                {
                        ...
                        t => new[] {
                            t.Column("Действия", (env, r) =>
                                new Link
                                {
                                    ...
                                    RouteValues = new MnuTelecomOperatorsTradeViewArgs { tradeId = r.GetVal(t => t.L.flId) },
