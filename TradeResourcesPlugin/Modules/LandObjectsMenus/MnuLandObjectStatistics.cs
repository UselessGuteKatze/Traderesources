...
namespace TradeResourcesPlugin.Modules.LandObjectsMenus {
    public class MnuLandObjectStatistics : FrmMenu {

        private static MemoryCache _memCache = new MemoryCache(new MemoryCacheOptions());

        public MnuLandObjectStatistics(string moduleName) : base(nameof(MnuLandObjectStatistics), "Статистика")
        {
            ...
            OnRendering(re => {
                ...
                var rows = _memCache.GetOrCreate(DateTime.Now.ToString("dd.MM.yyyy.") + (firstDayHalf ? "1" : "2"), (entry) => {
                    return new TbLandObjectsTrades().Self(out var tbLandObjectsTrades)
                    .JoinT(nameof(TbLandObjectsTrades), new TbLandObjects().Self(out var tbLandObjects), nameof(TbLandObjects), JoinType.Left)
                    .On((t1, t2) => new Join(t1.flObjectId, t2.flId))
                    .Select(t => new FieldAlias[] { t.L.flStatus, t.L.flDateTime, t.L.flCost, t.R.flRegion, t.R.flDistrict }, re.QueryExecuter);
                });
        ...
        private Card tradeCountGraphs(SelectResultProxy<QueryJoin<TbLandObjectsTrades, TbLandObjects>> rows)
        {
            var dateGroupedValues = rows
                    .Where(r => new[] { RefLandObjectTradeStatuses.Held, RefLandObjectTradeStatuses.Cancel }.Contains(r.GetVal(t => t.L.flStatus)))
            ...
            addChart("За последние 3 месяца", DateTime.Now.AddMonths(-3));
        ...
        private Card tradeSuccessPie(SelectResultProxy<QueryJoin<TbLandObjectsTrades, TbLandObjects>> rows)
        {
            ...
            var refSt = new RefLandObjectTradeStatuses();
            var seriesHour = statusGroupedValues.Select(r =>
                new Apex.PieDonut.SeriesInt()
                {
                    x = refSt.Search(r.Status).Text.Text,
                    y = r.Count
                }
            ).ToArray();
        ...
        private Card tradeWaitings(SelectResultProxy<QueryJoin<TbLandObjectsTrades, TbLandObjects>> rows)
        {
            var statusGroupedValues = rows
                    .Where(r => r.GetVal(t => t.L.flStatus) == RefLandObjectTradeStatuses.Wait).Count();
        ...
        private Card tradeSellCost(SelectResultProxy<QueryJoin<TbLandObjectsTrades, TbLandObjects>> rows)
        {
            var statusGroupedValues = rows
                    .Where(r => r.GetVal(t => t.L.flStatus) == RefLandObjectTradeStatuses.Held && r.GetValOrNull(t => t.L.flCost).HasValue).Sum(r => r.GetVal(t => t.L.flCost));
