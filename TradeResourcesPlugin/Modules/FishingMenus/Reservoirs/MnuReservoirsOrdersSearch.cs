...
namespace TradeResourcesPlugin.Modules.FishingMenus.Reservoirs {
    public class MnuReservoirsOrdersSearch: FrmMenu {

        public MnuReservoirsOrdersSearch(string moduleName) : base(nameof(MnuReservoirsOrdersSearch), "Приказы по водоёмам") {
            ...
            OnRendering(re => {
                ...
                var tbObjectsRev = new TbReservoirsRevisions();
                var xin = re.User.GetUserXin(re.QueryExecuter);
                var tbObjectsOrderResult = new TbReservoirOrderResult();
                            ...
                            t => new[] {
                                t.Column("Действия", (env, r) =>
                                    new Link
                                    {
                                        ...
                                        RouteValues = new ReservoirOrderQueryArgs { MenuAction = "view-order", RevisionId = r.GetVal(t => t.L.flRevisionId) },
