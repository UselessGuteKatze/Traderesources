...
namespace TradeResourcesPlugin.Modules.ForestMenus {
    public class MnuForestStatistics : FrmMenu {

        private static MemoryCache _memCache = new MemoryCache(new MemoryCacheOptions());

        public MnuForestStatistics(string moduleName) : base(nameof(MnuForestStatistics), "Статистика")
        {
        ...
        private Card tradeCountGraphs(SelectResultProxy<QueryJoin<TbTrades, TbObjects>> rows)
        {
            var dateGroupedValues = rows
                    .Where(r => new[] { TradesStatuses.Held, TradesStatuses.Cancel }.Contains(r.GetVal(t => t.L.flStatus)))
        ...
        {
            var statusGroupedValues = rows
                    .Where(r => r.GetVal(t => t.L.flStatus) == TradesStatuses.Wait).Count();
        ...
        {
            var statusGroupedValues = rows.Where(r => r.GetVal(t => t.L.flStatus) == TradesStatuses.Held && r.GetValOrNull(t => t.L.flCost).HasValue).Sum(r => r.GetVal(t => t.L.flCost));
