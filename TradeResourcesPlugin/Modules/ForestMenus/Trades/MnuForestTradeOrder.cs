using CommonSource.FieldEditors.Trade;
using CommonSource.References.Trade;
using ForestSource.FieldEditors.Trade;
using CommonSource;
...
namespace TradeResourcesPlugin.Modules.ForestMenus.Trades {
    public class MnuForestTradeOrder : MnuOrderBaseV2<ForestTradeModel, TradeOrderTypes, ForestTradeOrderQueryArgs> {

        public MnuForestTradeOrder(string moduleName, string menuName, OrderStandartPermissions perms)
        ...
        public override Task<ActionItem[]> GetActions(ActionEnv<ForestTradeOrderQueryArgs> env)
        {
            ...
            switch (env.Args.MenuAction)
            {
                case Actions.ViewOrder:
                    ...
                    if (trade != null)
                    {
                        ret.Add(new ActionItem(env.T("Перейти к объекту"), new ActionRedirectData(ModuleName, MnuForestObjectView.MnuName, new ForestObjectViewArgs { MenuAction = "view", Id = trade.flObjectId })));
                    }
                    break;
        ...
        public override IWizardFormBuilderWithStep<ForestTradeOrderQueryArgs, ModelOrderArgs<TradeOrderTypes, ForestTradeOrderQueryArgs>, ForestTradeModel>
            EditModel(IWizardFormBuilderWithFinishButton<ForestTradeOrderQueryArgs, ModelOrderArgs<TradeOrderTypes, ForestTradeOrderQueryArgs>, ForestTradeModel> wizard)
        {
            return wizard
                ...
                .OnProcessing(pe => {
                    ...
                    //pe.Model.flDeposit = tbRev.flDeposit.GetVal(pe.Env);
                    pe.Model.flDateTime = tbRev.flDateTime.GetVal(pe.Env);
                    ...
                    else
                    {
                        ...
                        var perc15 = (trade.flCostStart / 100) * 15;

                        pe.Model.flDeposit = (perc15 > mrp30000 ? mrp30000 : perc15 < mrp100 ? mrp100 : (trade.flCostStart / 100) * 15);
                    }
                ...
                .OnRendering(re => {
                    ...
                    tbRev.flDeposit.RenderCustom(re.Panel, re.Env, trade.flDeposit, readOnly: true,
                        description: new Label($"({re.Env.T("Начальная цена, тг.")} * 15% ({re.Env.T("минимум")} 100 {re.Env.T("мрп")} {mrp100} {re.Env.T("тг.")}, {re.Env.T("максимум")} 30000 {re.Env.T("мрп")} {mrp30000} {re.Env.T("тг.")}, 1 {re.Env.T("мрп")} = {mrp} {re.Env.T("тг.")}))")
                    );
                })
                .OnValidating(ve =>
                {
            ...
            #region Конкурс
                ...
                .OnValidating(ve => {
                    ...
                    if (ve.Env.IsValid) {
                        ...
                        if (flCommissionMembers.Length < 10) {
                            ve.Env.AddError(tbRev.flCommissionMembers.FieldName, ve.Env.T("Должно быть как минимум 9 членов комиссии (без секретаря)"));
                        }
                        ...
                        if (!flCommissionMembers.Any(x => {
                            return x[CommissionMembersEditor.Fls.flRole.ToString()].ToString() == CommissionMembersRoles.CHR.ToString();
                        })) {
                            ve.Env.AddError(tbRev.flCommissionMembers.FieldName, ve.Env.T("В комисси должен быть председатель"));
                        }
                    }
        ...
        public override ForestTradeModel GetModel(ActionEnv<ForestTradeOrderQueryArgs> env, GetModelFrom from, TradeOrderTypes orderType, ITransaction transaction)
        {
            ...
            {
                GetModelFrom.Empty => new ForestTradeModel()
                {
                    flCompetentOrgBin = env.Args.OwnerXin,
                    flDateTime = tradeDate,
                    flStatus = TradesStatuses.Wait,
            ...
            switch (orderType)
            {
                case TradeOrderTypes.Create:
                    model.flStatus = TradesStatuses.Wait;
                    break;
                case TradeOrderTypes.Cancel:
                    model.flStatus = TradesStatuses.CanceledEarlier;
                    break;
                default:
        ...
        public override TradeOrderTypes GetOrderType(ActionEnv<ForestTradeOrderQueryArgs> env, GetOrderModelFrom orderModelFrom)
        {
        ...
        public override string GetWizardCancelRedirectUrl(ActionEnv<ForestTradeOrderQueryArgs> env, TradeOrderTypes orderType, GetModelFrom modelFrom)
        {
            var urlHelper = env.RequestContext.GetUrlHelper();
            //TODO
            return modelFrom switch
            {
                GetModelFrom.Empty => urlHelper.YodaAction(ModuleName, nameof(MnuForestTradeOrder)),
                GetModelFrom.Entity => urlHelper.YodaAction(ModuleName, nameof(MnuForestTradeOrder), new ForestTradeOrderQueryArgs { Id = env.Args.Id.Value, MenuAction = "view" }),
                GetModelFrom.Revision => urlHelper.YodaAction(ModuleName, MenuName, new ForestTradeOrderQueryArgs { RevisionId = env.Args.RevisionId, MenuAction = Actions.ViewOrder }),
                _ => throw new NotImplementedException()
            };
        }

        public override ValidationResultBase IsCreateFromActionValid(ActionEnv<ForestTradeOrderQueryArgs> env) {
            if (env.Args.Id == null) {
            ...
            if (!CanCreateOrder(env.Args.Id.Value, GetOrderType(env, GetOrderModelFrom.Entity), out var error, env)) {
                if (env.Args.Id.HasValue) {
                    return new RedirectResult(ModuleName, MnuForestTradeView.MnuName, new MnuForestTradeViewArgs() { tradeId = env.Args.Id.Value }, error);
                }
                else {
        ...
        public static bool CanCreateOrder(int id, TradeOrderTypes orderType, out string error, ActionEnv<ForestTradeOrderQueryArgs> env)
        {
            ...
            {
                if (tradeResourceObject.flStatus != TradesStatuses.Wait)
                {
        ...
        public override void SaveModel(int revisionId, ForestTradeModel model, TradeOrderTypes orderType, ActionEnv<ForestTradeOrderQueryArgs> env, ITransaction transaction, GetModelFrom modelFrom)
        {
        ...
        private void setObjectStatus(TradeOrderTypes orderType, ForestTradeModel model, ActionEnv<ForestTradeOrderQueryArgs> env, ITransaction transaction)
        {
            switch (orderType)
            {
                case TradeOrderTypes.Edit:
            ...
            var objectBloc = ForestObjectBlocks.ActiveFree.ToString();
            if (orderType == TradeOrderTypes.Create)
            {
                objectBloc = ForestObjectBlocks.ActiveToBeTraded.ToString();
            }
            if (orderType == TradeOrderTypes.Cancel)
            {
                objectBloc = ForestObjectBlocks.ActiveFree.ToString();
            }
        ...
        private void setModel<TTradesTable>(DataModifingQueryProxy<TTradesTable> updateOrInsert, ForestTradeModel model) where TTradesTable : TbTradesBase
        {
            updateOrInsert
        ...
        public override bool TryAcceptModel(int revisionId, ForestTradeModel model, TradeOrderTypes orderType, ActionEnv<ForestTradeOrderQueryArgs> env, ITransaction transaction, out string error)
        {
            var result = true;
            ...
            if (orderType == TradeOrderTypes.Create)
            {
                if (new TbTrades().AddFilter(t => t.flObjectId, model.flObjectId).AddFilter(t => t.flStatus, TradesStatuses.Wait).Count(env.QueryExecuter) > 0)
                {
                    result = false;
                ...
                if (env.Args.MenuAction == MnuOrderV2Actions.Execute)
                {
                    ...
                    if (env.Args.MenuAction == MnuOrderV2Actions.Execute) {
                        var objectModel = new TbObjects()
                            .AddFilter(t => t.flId, model.flObjectId)
                        ...
                        else if (new[] { TradeOrderTypes.Create, TradeOrderTypes.Edit, TradeOrderTypes.Transfer }.Contains(orderType)) {
                            new TbForestryPieces().Self(out var tbForestryPieces);
                            tbForestryPieces.AddFilterIn(tbForestryPieces.flId, objectModel.flForestryPieces.Select(x => Convert.ToInt32(x.SearchItemId)).ToArray());
                            tbForestryPieces.Update()
                                .Set(t => t.flBlock, ForestryPieceBlocks.ActiveToBeTraded.ToString())
                                .Execute(env.QueryExecuter, transaction);
                        }
                    }
                }
        ...
        public override bool TryValidateModel(int revisionId, ForestTradeModel model, TradeOrderTypes orderType, ActionEnv<ForestTradeOrderQueryArgs> env, out string[] errors)
        {
            var result = true;
            var errorsList = new List<string>() { };
        ...
        private void setTradeChange(TradeOrderTypes orderType, ForestTradeModel model, ActionEnv<ForestTradeOrderQueryArgs> env, ITransaction transaction)
        {
        ...
        public override void ViewModel(RenderActionEnv<ForestTradeOrderQueryArgs> env, ForestTradeModel trade, TradeOrderTypes orderType, OrderStatus orderStatus)
        {
            ViewModel(env.Form, env, trade);
        }

        public static void ViewModel(WidgetBase widget, FormEnvironment env, ForestTradeModel trade)
        {
            viewModelMainData(widget, env, trade);

            viewModelRequisitesData(widget, env, trade);
            viewModelContestData(widget, env, trade);
        ...
        public static void viewModelMainData(WidgetBase widget, FormEnvironment env, ForestTradeModel trade, bool collapsed = false)
        {
            ...
            tbRevs.flDeposit.RenderCustom(groupbox, env, trade.flDeposit, readOnly: true,
                description: new Label($"({env.T("Начальная цена, тг.")} * 15% ({env.T("минимум")} 100 {env.T("мрп")} {mrp100} {env.T("тг.")}, {env.T("максимум")} 30000 {env.T("мрп")} {mrp30000} {env.T("тг.")}, 1 {env.T("мрп")} = {mrp} {env.T("тг.")}))")
            );
        ...
        public static void viewModelRequisitesData(WidgetBase widget, FormEnvironment env, ForestTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        public static void viewModelAgreementData(WidgetBase widget, FormEnvironment env, ForestTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        public static void viewModelSaleData(WidgetBase widget, FormEnvironment env, ForestTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        public static void viewModelContestData(WidgetBase widget, FormEnvironment env, ForestTradeModel trade, bool collapsed = false) {
            var tbRevs = new TbTradesRevisions();
            var groupbox = new Accordion(env.T("Условия конкурса"), collapsed);
            widget.AddComponent(groupbox);

            tbRevs.flParticipantConditions.RenderCustom(groupbox, env, trade.flParticipantConditions, readOnly: true);
            tbRevs.flRequiredDocuments.RenderCustom(groupbox, env, trade.flRequiredDocuments, readOnly: true);
            tbRevs.flCommissionMembers.RenderCustom(groupbox, env, trade.flCommissionMembers, readOnly: true);

        }

        public static void viewModelProtocolData(WidgetBase widget, FormEnvironment env, ForestTradeModel trade, bool collapsed = false)
        {
            var tbRevs = new TbTradesRevisions();
        ...
        protected override bool HasAccessToOrder(int revisionId, ActionEnv<ForestTradeOrderQueryArgs> env)
        {
    ...
    public class ForestTradeOrderQueryArgs : OrderQueryArgs {
