...
namespace FishingTradesToAuction {
    ...
    public class FishingTradesJobInput {
        public FishingTradesJobInput()
        {
        }
    }

    public class PgQueue {
        public const string FishingTradesToAuctionJobQueue = "FishingTradesToAuctionJobQueue";
        public const string WaitingFishingTradesFromAuctionJobQueue = "WaitingFishingTradesFromAuctionJobQueue";
        public const string HeldFishingTradesFromAuctionJobQueue = "HeldFishingTradesFromAuctionJobQueue";
        public const string FishingAgreementsToAuctionJobQueue = "FishingAgreementsToAuctionJobQueue";
    }

    public static class FishingTradesJobs {
        public static PgJobs<FishingTradesJobInput> FishingTradesToAuctionJob = new PgJobs<FishingTradesJobInput>(PgQueue.FishingTradesToAuctionJobQueue, NpGlobal.DbKeys.DbJobs, "Отправляет торги рыбхоз водоёмов");
        public static PgJobs<FishingTradesJobInput> WaitingFishingTradesFromAuctionJob = new PgJobs<FishingTradesJobInput>(PgQueue.WaitingFishingTradesFromAuctionJobQueue, NpGlobal.DbKeys.DbJobs, "Пытается забрать результаты ожидающихся торгов");
        public static PgJobs<FishingTradesJobInput> HeldFishingTradesFromAuctionJob = new PgJobs<FishingTradesJobInput>(PgQueue.HeldFishingTradesFromAuctionJobQueue, NpGlobal.DbKeys.DbJobs, "Перепроверяет результаты состоявшихся торгов");
        public static PgJobs<FishingTradesJobInput> FishingAgreementsToAuctionJob = new PgJobs<FishingTradesJobInput>(PgQueue.FishingAgreementsToAuctionJobQueue, NpGlobal.DbKeys.DbJobs, "Отправляет договоры");

        public static IServiceCollection AddFishingTradesToAuctionJobsConsumer(this IServiceCollection services)
        {
            return services.AddDbQueueConsumer(jobs => jobs
                .DbQueue(FishingTradesToAuctionJob)
        ...
        public static IServiceCollection AddWaitingFishingTradesFromAuctionJobsConsumer(this IServiceCollection services)
        {
            return services.AddDbQueueConsumer(jobs => jobs
                .DbQueue(WaitingFishingTradesFromAuctionJob)
                .Consume(async (input, env) =>
                {
                    try
                    {
        ...
        public static IServiceCollection AddHeldFishingTradesFromAuctionJobsConsumer(this IServiceCollection services)
        {
            return services.AddDbQueueConsumer(jobs => jobs
                .DbQueue(HeldFishingTradesFromAuctionJob)
                .Consume(async (input, env) =>
                {
                    try
                    {
        ...
        public static IServiceCollection AddFishingAgreementsToAuctionJobsConsumer(this IServiceCollection services)
        {
            return services.AddDbQueueConsumer(jobs => jobs
                .DbQueue(FishingAgreementsToAuctionJob)
                .Consume(async (input, env) =>
                {
                    try
                    {
    ...
    public class JobSource {
        ...
        public const string ReSource = "AnimalWorldFishing"; // СУРС
        public static string AuctionObjectType = "FishingReservoir"; // ТИП ОБЪЕКТА
        ...
        public static void CheckHeldTrades(DbJobConsumeEnvironment env)
        {
            ...
            if (dtTradeChanges.Rows.Count > 0)
            {
                foreach (DataRow row in dtTradeChanges.Rows)
                {
                    try
                    {
                        ProcessCheckHelds(tbTradeChanges.flTradeId.GetRowVal(row), env, service);
                    }
                    catch (Exception e)
                    {
                        env.Logger.Log(
                        LogLevel.Error,
                        @$"Error while checking held trade {tbTradeChanges.flTradeId.GetRowVal(row)}: 
                            {e.Message}
                            {e.StackTrace}"
                        );
                    }
        ...
        public static void ProcessEAuction(FishingTradeModel tradeModel, DbJobConsumeEnvironment env, AuctionServiceSoapClient service, LanguageTranslatorCore langTranslator)
        {
            ...
            var metaData = new CommonSource.Auction.MetaData
            {
                ...
                {
                    LandUrl = GetObjectOnMapUrl(objectModel.flId, "FISHINGOBJECT")
                }
            };
        ...
        public static void SendAuction(AuctionData auctionData, FishingTradeModel tradeModel, FishingObjectModel objectModel, DbJobConsumeEnvironment env, AuctionServiceSoapClient service)
        {
        ...
        public static void ProcessCheckWaitings(int tradeId, DbJobConsumeEnvironment env, AuctionServiceSoapClient service)
        {
            ...
            var agreementModel = AgreementHelper.GetAgreementModel(objectId, "FishingReservoir", tradeId, "CompetitionHunt", env.QueryExecuter);
                    ...
                    case "Success":
                        {
                            using (var transaction = env.QueryExecuter.BeginTransaction())
                            {
                                ...
                                if (result.Data.Data != null)
                                {
                                    ...
                                    {
                                        if (!new[] { FishingObjectBlocks.SaledProt.ToString(), FishingObjectBlocks.SaledAgr.ToString() }.Contains(objectBlock))
                                        {
                                            ObjectHelper.SetBlock(objectId, FishingObjectBlocks.SaledProt, env.QueryExecuter, transaction);
                                        }
                    ...
                    case "Canceled":
                        {
                            using (var transaction = env.QueryExecuter.BeginTransaction())
                            {
                                ...
                                {
                                    ObjectHelper.SetBlock(objectId, FishingObjectBlocks.ActiveFree, env.QueryExecuter, transaction);
                                }
        ...
        private static AuctionData getAuction(string source, FishingTradeModel trade, AuctionObjectModel auctionObject, string AuctionType, CommonSource.Auction.MetaData metaData, AuctionCommissionModel[] commissions = null, AuctionRequiredFilesGroupModel[] requieredFiles = null,
        ...
        private static ObjectInfo getObjectInfo(FishingObjectModel objectModel, FishingTradeModel tradeModel, IQueryExecuter queryExecuter, ITransaction transaction, ILangTranslator langTranslator)
        {
            ...
            sbFullInfoRu.AppendFormat($"Физическая характеристика: ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate("Физическая характеристика", "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flLength.Text}: {objectModel.flLength}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flLength.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {objectModel.flLength}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flWidth.Text}: {objectModel.flWidth}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flWidth.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {objectModel.flWidth}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flArea.Text}: {objectModel.flArea}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flArea.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {objectModel.flArea}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flMaxDepth.Text}: {objectModel.flMaxDepth}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flMaxDepth.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {objectModel.flMaxDepth}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flAvgDepth.Text}: {objectModel.flAvgDepth}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flAvgDepth.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {objectModel.flAvgDepth}; ");



            var propertyLevelValue = new RefPropertyLevel();
            var refFauna = new RefFaunaDetailed();
            sbFullInfoRu.AppendFormat($"Биологическая характеристика: ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate("Биологическая характеристика", "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flEmergentVegetation.Text}: {propertyLevelValue.Search(objectModel.flEmergentVegetation).Text} ({objectModel.flEmergentVegetationValue}%); ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flEmergentVegetation.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(propertyLevelValue.Search(objectModel.flEmergentVegetation).Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")} ({objectModel.flEmergentVegetationValue}%); ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flUnderwaterVegetation.Text}: {propertyLevelValue.Search(objectModel.flUnderwaterVegetation).Text} ({objectModel.flUnderwaterVegetationValue}%); ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flUnderwaterVegetation.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(propertyLevelValue.Search(objectModel.flUnderwaterVegetation).Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")} ({objectModel.flUnderwaterVegetationValue}%); ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flPhytoplankton.Text}: {propertyLevelValue.Search(objectModel.flPhytoplankton).Text} ({objectModel.flPhytoplanktonValue}%); ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flPhytoplankton.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(propertyLevelValue.Search(objectModel.flPhytoplankton).Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")} ({objectModel.flPhytoplanktonValue}%); ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flFauna.Text}: {ReferenceHelper.GetGroupedString(refFauna, objectModel.flFauna)}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flFauna.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(ReferenceHelper.GetGroupedString(refFauna, objectModel.flFauna), "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flСommercialFauna.Text}: {ReferenceHelper.GetGroupedString(refFauna, objectModel.flСommercialFauna)}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flСommercialFauna.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(ReferenceHelper.GetGroupedString(refFauna, objectModel.flСommercialFauna), "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}; ");

            var fishProductivityEditor = new FishProductivityEditor(null, null, null);
            sbFullInfoRu.AppendFormat($"{TbObjects.flFishProductivity.Text}: {fishProductivityEditor.GetDisplayString(objectModel.flFishProductivity, null)}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flFishProductivity.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {fishProductivityEditor.GetDisplayString(objectModel.flFishProductivity, null)}; ");


            sbFullInfoRu.AppendFormat($"Хозяйственная характеристика: ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate("Хозяйственная характеристика", "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flPollution.Text}: {objectModel.flPollution}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flPollution.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(objectModel.flPollution, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flHasWaterIntakes.Text}: {(objectModel.flHasWaterIntakes ? "Да" : "Нет")}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flHasWaterIntakes.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate((objectModel.flHasWaterIntakes ? "Да" : "Нет"), "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}; ");

            if (objectModel.flHasWaterIntakes)
            {
                var waterIntakesEditor = new WaterIntakesEditor(null, null, null);
                sbFullInfoRu.AppendFormat($"{TbObjects.flWaterIntakes.Text}: {waterIntakesEditor.GetDisplayString(objectModel.flWaterIntakes, null)}; ");
                sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flWaterIntakes.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {waterIntakesEditor.GetDisplayString(objectModel.flWaterIntakes, null)}; ");
            }

            sbFullInfoRu.AppendFormat($"{TbObjects.flPermanentUsingPlacesCount.Text}: {objectModel.flPermanentUsingPlacesCount}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flPermanentUsingPlacesCount.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {objectModel.flPermanentUsingPlacesCount}; ");

            sbFullInfoRu.AppendFormat($"{TbObjects.flAnotherInformation.Text}: {objectModel.flAnotherInformation}; ");
            sbFullInfoKz.AppendFormat($"{langTranslator.Translate(TbObjects.flAnotherInformation.Text, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}: {langTranslator.Translate(objectModel.flAnotherInformation, "kz", "Reference:TraderesourcesAuctionIntegrationMonitor")}; ");
        ...
        private static AuctionCommissionModel[] getCommission(FishingTradeModel trade, IQueryExecuter queryExecuter, ITransaction transaction)
        {
        ...
        private static AuctionRequiredFilesGroupModel[] getRequiredFiles(FishingTradeModel trade)
        {
            ...
            userTypes.ForEach(userType =>
            {
                ...
                {
                    requiredFiles.Add(new AuctionRequiredFilesGroupModel { GroupName = $"Fishing_{userType}_{trade.flId}_{index}", GroupText = d["flDocument"].ToString(), ForUserType = userType });
                    index++;
                });
            });
            return requiredFiles.ToArray();
        }
        private static AuctionRequirementsToParticipantModel[] getRequirements(FishingTradeModel trade)
        {
            ...
            {
                requirements.Add(new AuctionRequirementsToParticipantModel { RequirementId = $"Fishing_{trade.flId}_{index}", Text = c["flCondition"].ToString() });
                index++;
            });
            //});
            return requirements.ToArray();
