...
(function ($) {
    ...
    $.fn.fileUploader = function (config) {
        ...
        config = $.extend({}, {
            ...
            allowedExtension: '.',
            invalidFileErrorMsg:"Invalid File!",
        ...
        function uploadChange($this) {
            ...
            if (px.validateFileName(filename) == -1) {
                if ($.isFunction(config.onValidationError)) {
                    ...
                    alert(config.invalidFileErrorMsg);
                }
                $form.find(':file').val('');
                return false;
            }
            ...
            //Limit
            if (limit <= 0) {
                alert("Нельзя добавлять более одного файла");
                return false;
            }
            limit = limit - 1;
            ...
            $(pxUploadForm).append(
				...
				.append(' \
					<ul class="actions ui-helper-clearfix"> \
						<li title="Загрузить" class="upload ui-state-default ui-corner-all"> \
							<span class="ui-icon ui-icon-circle-triangle-e"></span> \
						</li> \
						<li title="Удалить" class="delete ui-state-default ui-corner-all"> \
					...
					<div class="status">Ожидание загрузки...</div> \
				')
			);
        ...
        */
        function beforeEachUpload($form) {
            ...
            $uploadData = $form.parent();
            $uploadData.find('.status').text('Загружается...');
        ...
        */
        function afterEachUpload(formId, data, status, errorThrown) {
            ...
            if (status == 'success') {
                ...
                $uploadData.children('.status').html('Загрузка отменена');
            }

            $uploadData.find('.cancel').removeClass('cancel').addClass('delete').attr('title', 'Удалить');
